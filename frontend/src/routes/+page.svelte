<script lang="ts">
	...
	let buttonLabel = $derived(
		uploadStatus === 'uploading' ? 'Uploading…' : 'Upload OpenAPI spec File'
	);
	...
	async function upload() {
		...
		try {
			...
			const res = await fetch(`${PUBLIC_API_PREFIX}/upload`, {
				method: 'POST',
				body: formData
			});
	...
	<div class="w-full max-w-lg space-y-8">
		...
		<header class="text-center">
			...
			<p class="mt-2 text-sm text-neutral-400">
				Upload an OpenAPI specification to normalize and process it.
			</p>
		</header>
		...
		{:else}
			...
			>
				<input
					type="file"
					accept=".json,application/json,.yaml,application/yaml"
				...
				{:else}
					...
					<div class="text-center">
						<p class="text-sm font-medium text-neutral-300">Drop your spec file here</p>
						<p class="mt-1 text-xs text-neutral-500">or click to browse — .json/.yaml only</p>
					</div>
				{/if}
			</label>
